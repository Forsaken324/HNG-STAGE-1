...
def is_amstrong(n : int) -> bool:
   return n == sum(int(digit) ** len(str(n)) for digit in str(abs(n)) if digit.isdigit())

def get_fun_fact(n : int) -> str:
    # make a request to the numbers api and return a fun fact about that number
    session = requests.Session()
    try:
        response = session.get(f'http://numbersapi.com/{n}/math?json')
...
def classify_number(request):
    ...
    # sum digits is the sum of its digitsz
